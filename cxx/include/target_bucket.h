...
struct TargetBucket {
    ...
    TargetBucket(float alpha) : buckets_(),
                    map_bucket_index_(),
    ...
    std::vector<std::pair<int32_t, DiffType>> Diffs() {
        std::vector<std::pair<int32_t, DiffType>> diffs(diffs_.begin(), diffs_.end());
        return diffs;
    }
    ...
    // The cost of stashing this bucket. If < 0, it's better to make this bucket an outlier.
    float CostToStash(const MapBucket& mb) const {
        ...
        return outlier_cost - inlier_cost;
    }
    ...
    void AddPointsBatch(std::vector<std::pair<int32_t, int32_t>> buckets) {
        for (const auto& b : buckets) {
            ...
            if (loc == map_bucket_index_.end()) {
                MapBucket mb = {.index = b.first, .num_points = b.second, .is_outlier = false};
        ...
        num_inlier_points_ = 0;
        num_inlier_buckets_ = buckets_.size();
        // Build the map_bucket_index_ map from scratch.
        // Reset every bucket to be an inlier and compute outliers from scratch.
        for (size_t i = 0; i < buckets_.size(); i++) {
            map_bucket_index_[buckets_[i].index] = i;
            buckets_[i].is_outlier = false;
            num_inlier_points_ += buckets_[i].num_points;
        }
        first_inlier_index_ = 0;
        Restash();
    }
