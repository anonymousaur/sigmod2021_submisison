...
CompressedColumnOrderDataset<D>::CompressedColumnOrderDataset(const std::vector<Point<D>>& data,
    ...
    for (size_t i = 0; i < data.size(); i++) {
        ...
        // If row IDs are provided, use those as the primary key. Otherwise,
        // just number the entry.
        if (row_ids.size() > 0) {
            columns[D].push_back(row_ids[i]);
        } else {
            columns[D].push_back(i);
        }

        for (size_t i = 0; i < cubes_.size(); i++) {
            Datacube<D> dc = cubes_[i];
