...
PhysicalIndexSet PrimaryBTreeIndex<D>::Ranges(const Query<D>& q) const {
    ...
    if (accessed.is_range) {
        ...
        for (ScalarRange r : accessed.ranges) {
            const PhysicalIndexRange pr = PageRangeFor(r.first, r.second);
        ...
        for (Scalar val : accessed.values) {
            const PhysicalIndexRange pr = PageRangeFor(val, val);
            if (added > 0 && pr.start == ranges[added-1].end) {
                ranges[added-1].end = pr.end;
            } else if (pr.end > pr.start) {
                ranges.push_back(pr);
...
template <size_t D>
void PrimaryBTreeIndex<D>::Init(PointIterator<D> start, PointIterator<D> end) {
