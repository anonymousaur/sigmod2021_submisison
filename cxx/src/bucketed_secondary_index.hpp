...
template <size_t D>
BucketedSecondaryIndex<D>::BucketedSecondaryIndex(size_t dim, IndexList subset)
    : BucketedSecondaryIndex<D>(dim, true, subset) {}

template <size_t D>
BucketedSecondaryIndex<D>::BucketedSecondaryIndex(size_t dim, bool use_subset, IndexList subset)
...
void BucketedSecondaryIndex<D>::SetBucketFile(const std::string& filename) {
    ...
    for (size_t i = 0; i < s; i++) {
        ...
        buckets_.insert(std::make_pair(bucket_start, IndexList{}));
    }
    file.close();
...
template <size_t D>
void BucketedSecondaryIndex<D>::InsertUnsorted(size_t index, Scalar v)  {
    ...
    if (bucket_strat_ == CONST_WIDTH) {
        ...
        loc = buckets_.insert(std::make_pair(key, IndexList{})).first;
...
template <size_t D>
IndexList BucketedSecondaryIndex<D>::Matches(const Query<D>& q) const {
    ...
    std::vector<const IndexList *> idx_lists;
    size_t max_size = 0;
    ...
    if (idx_lists.size() <= 5) {
        ...
        IndexList all_ixs;
        all_ixs.reserve(max_size);
        for (const IndexList* ixl : idx_lists) {
